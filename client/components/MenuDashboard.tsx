...
export default function MenuDashboard({ onClose }: MenuDashboardProps) {
  ...
  const cacheKey = user
    ? `menuStats:${user._id || (user as any).id || "me"}:${user.userType}`
    : "";
  useEffect(() => {
    if (!cacheKey) return;
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("seller:dashboard:refresh", fn as any);
  }, []);
  ...
  async function fetchAll() {
    try {
      ...
      const computed: Stats = {
        ...
        pendingApproval: props.filter((p) => p.approvalStatus === "pending")
          .length,
      ...
      if (rNoti.status === "fulfilled" && rNoti.value?.data?.success) {
        ...
        computed.unreadNotifications = Array.isArray(arr)
          ? arr.length
          : Number(arr) || 0;
      }
      ...
      if (
        rStats.status === "fulfilled" &&
        rStats.value?.data?.success &&
        rStats.value.data.data
      ) {
        const s = rStats.value.data.data;
        computed.totalProperties =
          s.totalProperties ?? computed.totalProperties;
        computed.pendingApproval =
          s.pendingApproval ?? computed.pendingApproval;
        ...
        computed.unreadNotifications =
          s.unreadNotifications ?? computed.unreadNotifications;
      }
        ...
        {user?.userType === "agent" && (
          <div className="grid grid-cols-2 gap-3 mb-4">
            ...
            <StatBox
              color="orange"
              label="Commission"
              value={`₹${(stats.commission / 1000).toFixed(0)}K`}
            />
          </div>
        )}
        {user?.userType === "buyer" && (
            ...
            <div className="mb-4">
              ...
              <div className="space-y-2">
                {properties.slice(0, 3).map((property) => (
                  <div
                    key={(property as any)._id || (property as any).id}
