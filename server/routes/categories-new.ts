...
/**
 * Helper: active filter compatible with both {isActive:true} and {active:true}
 */
function buildActiveOrFilter(active?: any) {
  if (String(active) !== "true") return null;
  return { $or: [{ isActive: true }, { active: true }] };
}

/**
 * Helper: attach miniSubcategories for each subcategory
 */
async function attachMiniSubcategories(
  db: any,
  subcategoriesRaw: any[],
  active?: any,
) {
  const activeOr = buildActiveOrFilter(active);

  return Promise.all(
    (subcategoriesRaw || []).map(async (sub: any) => {
      const miniFilter: any = { subcategoryId: sub._id.toString() };
      if (activeOr) {
        miniFilter.$or = activeOr.$or;
      }
...
export const getCategories: RequestHandler = async (req, res) => {
  try {
    ...
    // Include subcategories + miniSubcategories if requested
    if (withSub === "true") {
      result = await Promise.all(
        categories.map(async (category: any) => {
          const subFilter: any = { categoryId: category._id.toString() };
          if (activeOr) {
            subFilter.$or = activeOr.$or;
          }

          const subcategoriesRaw = await db
            .collection("subcategories")
            .find(subFilter)
            .sort({ sortOrder: 1, createdAt: 1 })
            .toArray();

          const subcategories = await attachMiniSubcategories(
            db as any,
            subcategoriesRaw,
            active,
          );
...
// PUBLIC: Get category by slug with subcategories (+ miniSubcategories)
...
// PUBLIC: Get subcategories by category slug (+ miniSubcategories)
    ...
    const categoriesWithCounts = await Promise.all(
      categories.map(async (category: any) => {
        ...
        if (withSub === "true") {
          ...
          subcategories = await attachMiniSubcategories(
            db as any,
            subcategoriesRaw,
            "false",
          );
